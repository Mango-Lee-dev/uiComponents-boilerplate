...
const getVisibleEdgeItems = (
  ...
  const isVisible = ($item: ItemElemType) => {
    ...
    return left <= lRight && right >= lLeft; // 애매하게 걸친 경우까지 인정하는 조건
  };
...
export type ScrollBoxHandle =
  | {
      scrollFocus: (index: number, behavior?: "smooth" | "instant") => void;
    }
  | null
  | undefined;
...
const ScrollBox = <T extends { id: string }>(
  ...
  const move = useCallback((direction: Direction) => {
    ...
    elem?.scrollIntoView({
      inline: direction === "prev" ? "start" : "end",
      block: "nearest",
      behavior: "smooth",
    });
  }, []);
...
const ForwardedScrollBox = forwardRef(ScrollBox) as <T extends { id: string }>(
  props: ScrollBoxProps<T> & { ref: ForwardedRef<unknown> },
) => JSX.Element;
